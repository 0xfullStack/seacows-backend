...
type MultipleCollectionsResponse =
...
export class ReservoirHttpApi {
  ...
  async searchCollections(name: string) {
    ...
    const data =
      await this.ReservoirHttpClient.makeRequest<SearchCollectionsResponse>(
  ...
  async requestCollections(
    collectionId: string
  ): Promise<ReservoirCollectionResponse> {
  ...
  async requestCollectionAllAttributes(
    ...
    const data =
      await this.ReservoirHttpClient.makeRequest<CollectionAllAttributesResponse>(
  ...
  async requestMultipleCollections(
    ...
    const data =
      await this.ReservoirHttpClient.makeRequest<MultipleCollectionsResponse>(
        "collections/v5",
        {
          searchParams: {
            sortBy,
  ...
  async requestTokenAttributes(collectionId: string, tokenId: string) {
    const data =
      await this.ReservoirHttpClient.makeRequest<ExploreAttributesResponse>(
  ...
  async requestMaxTokens(
    ...
    const { continuation: continueToken, tokens } =
      await this.requestCollectionTokens(collectionId, "tokenId", continuation);
    ...
    if (continueToken) {
      ...
      const nextTokens = await this.requestMaxTokens(
        collectionId,
        continueToken
      );
